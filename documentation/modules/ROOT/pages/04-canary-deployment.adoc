...
To allow communication between the frontend and the backend, it is required to edit the file named *./canary/dstrategies-frontend-deployment.yaml* to add the required **<USERNAME>** and **<OCP_DOMAIN>**. You can check their values in the xref:01-setup.adoc#01-parameters[parameters chapter].

[source,yaml,subs="+macros,+attributes"]
----
        env:  
          - name: REACT_APP_BACK
            value: http://dstrategies-back-online-<USERNAME>-canary.apps.<OCP_DOMAIN>/dstrategies-back
...
We have defined an active or online service 'dstrategies-back-online'. Final users will always use 'dstrategies-back-online'. When a new version is deployed **Argo Rollouts** creates a new revision (ReplicaSet). The number of replicas in the new release increases based on the information in the steps, and the number of replicas in the old release decreases by the same number. We have configured a pause duration between each step. To learn more about **Argo Rollouts**, please read https://argoproj.github.io/argo-rollouts/features/canary/[this].
